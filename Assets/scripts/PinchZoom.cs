...
public class PinchZoom : MonoBehaviour
{
	...
	void Start()
	{
		...
		//logic 
		targetScale = parentObject.transform.localScale;
	...
	void Update()
	{
		...
		// Smooth zoom
		parentObject.transform.localScale = Vector3.Lerp(parentObject.transform.localScale, targetScale, Time.deltaTime * zoomSpeed);
	...
	private void checkForMultiTouch()
	{
		...
		if (Input.touchCount == 2 && Input.GetTouch(0).phase == TouchPhase.Moved && Input.GetTouch(1).phase == TouchPhase.Moved)
		{
			...
			// Zoom out
			float zoomDelta = touchDelta * 0.01f; // sensitivity control
			float newScaleX = Mathf.Clamp(parentObject.transform.localScale.x + zoomDelta, MIN_SCALE, MAXSCALE);
			float newScaleY = Mathf.Clamp(parentObject.transform.localScale.y + zoomDelta, MIN_SCALE, MAXSCALE);
			targetScale = new Vector3(newScaleX, newScaleY, 1);
		}
...
{
    float xBound = ScreenSize.x * (parentObject.transform.localScale.x - 1);
    float yBound = ScreenSize.y * (parentObject.transform.localScale.y - 1);
